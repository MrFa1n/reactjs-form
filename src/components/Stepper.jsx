...
const Stepper = ({ steps, currentStep }) => {
        ...
        return (
            <div key={index} className={index !== newStep.length - 1 ? "w-full flex items-center" : "flex items-center"} >
                <div className="relative flex flex-col items-center text-teal-600">
                    ...
                    <div className={`absolute top-0 text-center mt-16 w-16 text-xs font-medium uppercase ${step.highlighted ? "text-gray-900" : "text-gray-400"}`}>
                        {/* Display description */}
                        {/* Fist step */}
                        {step.description}
                    </div>
    ...
    return (
        <div className="mx-4 p-4 flex justify-between items-center flex-col:max-w-sm">
            {displaySteps}
        </div>
    )
}
